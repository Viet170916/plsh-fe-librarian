...
export const appToaster = {
    success(message: React.ReactNode, position: ToastPosition = "top-right", duration: number = 2000) {
        toast.message(() => (
            <Grid sx={{background: color.WHITE, borderRadius: 4, width: "100%", minWidth: 350}}>
                <Alert icon={<LuCircleCheck color={color.COMFORT}/>} variant="outlined" sx={{
                    color: color.COMFORT,
    ...
    warning(message: React.ReactNode, position: ToastPosition = "top-right", duration: number = 2000) {
        toast.message(() => (
            <Grid sx={{background: color.WHITE, borderRadius: 4, width: "100%", minWidth: 350}}>
                <Alert icon={<TiWarningOutline color={color.WARNING}/>} variant="outlined" sx={{
                    color: color.WARNING,
                    borderColor: color.WARNING,
                    bgcolor: color.WARNING_10,
                    borderRadius: 4
                }}>
                    {message}
                </Alert>
    ...
    error(message: React.ReactNode, position: ToastPosition = "top-right", duration: number = 2000) {
        toast.message(() => (
            <Grid sx={{background: color.WHITE, borderRadius: 4, width: "100%", minWidth: 350}}>
                <Alert icon={<LuSquareX color={color.SERIOUS}/>} variant="outlined" sx={{
                    color: color.SERIOUS,
    ...
    info(message: React.ReactNode, position: ToastPosition = "top-right", duration: number = 6000) {
        toast.message(() => (
            <Grid sx={{background: color.WHITE, borderRadius: 4, width: "100%", minWidth: 350}}>
                <Alert icon={<GoInfo color={color.PRIMARY}/>} variant="outlined" sx={{
                    color: color.PRIMARY,
                    borderColor: color.PRIMARY,
                    bgcolor: color.PRIMARY_O10,
                    borderRadius: 4
                }}>
                    {message}
                </Alert>
    ...
    custom(message: React.ReactNode, position: ToastPosition = "top-right", duration: number = 6000) {
