...
function UploadOrTakeImage({onImageChange, maxImages}: IProps) {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (images.length + files.length > maxImages) {
            toast.warning(`Bạn chỉ có thể tải lên tối đa ${maxImages} ảnh.`);
            return;
        }
    ...
    return (
        <Grid className="p-4 space-y-4" container width={"100%"} justifyContent="center" alignItems="center">
            {images.length < maxImages &&
                <>
                    <Grid size={6}>
                        <Button sx={{color: color.LIGHT_TEXT}} component={"label"} variant={"contained"}
                                startIcon={<TbPhotoUp/>}>
                    ...
                    <Grid size={3} sx={{margin: "0!important"}}>
                        <ModalPanel close={triggerClose} buttonContent={<PhotoCameraIcon/>}>
                            <TakePicture onImageChange={(preview) => {
                                if (preview && images.length < MAX_IMAGES) {
                    ...
                    <Stack direction={"row"} useFlexGap spacing={2}>
                        {images.map((src, index) => (
                            <Box key={src.name} position={"relative"}>
                                ...
                                <IconButton
                                    onClick={() => {
                                        setImages(prev => prev.filter(im => im !== src))
                                    }}
                                    sx={{
                                        top: 0,
                                        right: 0,
