...
import {FieldPathValue, Path, set} from "react-hook-form";
import {PayLoad} from "@/stores/slices/notification/notification.slice";
// type
...
type AddEditBookDataSlice = Slice<AddEditBookData, {
    setPropToAddEditBook: <K extends Path<AddEditBookData>>(state: WritableDraft<AddEditBookData>, action: PayloadAction<PayLoad<AddEditBookData, K>>) => void,
...
const addEditBookDataSlice: AddEditBookDataSlice = createSlice({
    ...
    reducers: {
        setPropToAddEditBook(state, {payload: {key, value}}) {
            set(state, key, value);
        },
        setValueInBookBaseInfo: (state: WritableDraft<AddEditBookData>, action: PayloadAction<{
            key: keyof BookBaseInfo,
