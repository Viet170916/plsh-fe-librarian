...
import {BookData, Member, Payload, Resource} from "@/helpers/appType";
...
export const initAppGlobal: AppGlobal = {
    ...
    libRoom_g: [],
    images: [],
    me: {role: "librarian", status: "active"},
};
type GlobalSlice = Slice<AppGlobal, {
    setPropToGlobal: <K extends Path<AppGlobal>>(
        state: WritableDraft<AppGlobal>,
        action: PayloadAction<Payload<AppGlobal, K>>,
    ) => void;
...
const globalSlice: GlobalSlice = createSlice({
    ...
    reducers: {
        setPropToGlobal(state, {payload: {key, value}}) {
            set(state, key, value);
        },
