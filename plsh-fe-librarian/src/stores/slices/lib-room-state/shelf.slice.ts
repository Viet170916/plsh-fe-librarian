...
type ShelfStateSlice = Slice<ShelfState, {
				...
				setValueInRow: ( state: WritableDraft<ShelfState>, action: PayloadAction<{ rowId: number, key: keyof RowShelf, value: FieldPathValue<RowShelf, keyof RowShelf> }> ) => void
...
const shelfStateSlice: ShelfStateSlice = createSlice( {
								...
								reducers: {
												...
												setBookPosition: ( shelfState: WritableDraft<ShelfState>, { payload: { rowId, bookOnShelfId, newPosition } }: PayloadAction<{
																rowId: number,
																bookOnShelfId: number,
																newPosition: number
												}> ) => {
												...
												setValueInRow: <K extends keyof RowShelf>( shelfState: WritableDraft<ShelfState>, { payload: { rowId, key, value } }: PayloadAction<{ rowId: number, key: K, value: RowShelf[K] }> ) => {
																const row = shelfState.rowShelves?.find( r => r.id === rowId );
																if( row ){
																				row[key] = value;
																}
