...
import {BookInstance, Payload} from "@/helpers/appType";
...
type ShelfStateSlice = Slice<LibraryRoomState, {
    setPropToLibRoom: <K extends Path<LibraryRoomState>>(state: WritableDraft<LibraryRoomState>, action: PayloadAction<Payload<LibraryRoomState, K>>) => void
    ...
    setBookToRow: (state: WritableDraft<LibraryRoomState>, action: PayloadAction<{
        rowId: number,
        shelfId: number,
        value: BookInstance[]
    }>) => void
    addBooksToRow: (state: WritableDraft<LibraryRoomState>, action: PayloadAction<{
        rowId: number,
        shelfId: number,
        value: BookInstance[],
...
const libraryRoomStateSlice: ShelfStateSlice = createSlice({
        ...
        reducers: {
            setPropToLibRoom(state, {payload: {key, value}}) {
                set(state, key, value);

            },
            setBookToRow: (state: WritableDraft<LibraryRoomState>, {payload: {rowId, shelfId, value}}: PayloadAction<{
                rowId: number,
                shelfId: number,
                value: BookInstance[]
            }>) => {
            ...
            addBooksToRow: (state: WritableDraft<LibraryRoomState>, {payload: {rowId, shelfId, value}}: PayloadAction<{
                rowId: number,
                shelfId: number,
                value: BookInstance[]
            }>) => {
...
export const selectRowByIdInLibStore = createSelector(
    ...
    (shelves: Shelf[], param: {
        shelfId?: number,
        rowId?: number
    }) => shelves.find((s) => s.id === param.shelfId)?.rowShelves.find(r => r.id === param.rowId));
