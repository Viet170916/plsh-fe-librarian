"use client";
import { RowShelf_Container } from "@/app/(private)/(in-dash-board)/resources/library-room/shelf/row-component/container.RowShelf";
...
import { openDialog } from "@/stores/slices/global.slice";
...
const RowShelf = ( { rowId }: { rowId: number } ) => {
				...
				async function handleDeleteRow(){
								...
								if( anyBookResponse.data?.hasBooks ){
												...
												try{
																...
																toast.success( appStrings.error.REMOVE_FAIL );
												}
								}
				}
				...
				const handleCancel = useCallback( () => {
								toast.info( appStrings.CANCELED_DELETE );
				}, [] );
				return (
								<Box sx = { { width: "100%", borderRadius: 5, border: "1px solid #ccc", padding: 2, position: "relative" } }>
												...
												<Box sx = { { display: "flex", gap: 1, marginBottom: 2 } }>
																...
																<Button variant = "outlined" onClick = { handleDeleteRow }>{ appStrings.REMOVE }</Button>
												</Box>
												<Snackbar
