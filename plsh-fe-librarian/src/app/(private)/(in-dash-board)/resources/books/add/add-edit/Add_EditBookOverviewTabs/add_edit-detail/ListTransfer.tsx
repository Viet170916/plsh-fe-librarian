        ...
        <Box>
            <List dense component="div" role="list">
                {items.length > 0 ? items.map((value: BookInstance) => {
                        ...
                        >
                            <ListItemText
                                ...
                                <Typography variant={"h6"}
                                            color={"textPrimary"}>{`${appStrings.book.BOOK_CODE}: ${value.code}`}</Typography>}
                                secondary={<Typography variant={"h6"} fontWeight={"lighter"}
                                                       color={"textPrimary"}>{value.shelfPosition}</Typography>}
                            />
                            <ListItemIcon>
                                <Checkbox
                    ...
                    {appStrings.shelf.SHELF_EMPTY_WITH_BOOK}
                </Typography>}
            </List>
        </Box>
    );
