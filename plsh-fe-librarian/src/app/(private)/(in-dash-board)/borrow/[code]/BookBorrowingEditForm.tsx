...
const AfterBorrowField = memo( <K extends Path<LoanState>>( { name, label, type }: { name: K, label: string, type?: "text" | "date-time" } ) => {
								...
								<Drawer anchor = "right" open = { selectedBookBorrowing && true } onClose = { onClose }>
												<Box width = { "50vw" } p = { 3 }>
																...
																<Grid container width = { "100%" } spacing = { 2 }>
																				...
																				<Grid size = { 12 }>
																								{ selectedBookBorrowing &&
                          <LineChart
                            // yAxis = { [ { data: [ 1, 2, 3, 5, 8, 10, 12, 15, 16 ] } ] }
																														// series = { getOnLoanPeriods( dayjs( selectedBookBorrowing?.borrowDate ),
																														// 				selectedBookBorrowing?.returnDates.map( d => dayjs( d ) ) ?? [],
																														// 				selectedBookBorrowing?.extendDates.map( d => dayjs( d ) ) ?? [],
																														// 				dayjs( selectedBookBorrowing?.actualReturnDate ?? dayjs() ) )
																														// 				.chartData
																														// }
                            series = {
																																getOnLoanPeriods( dayjs( "2024-03-01" ), [
																																				dayjs( "2024-03-10" ),
																																				dayjs( "2024-03-20" ),
																																				dayjs( "2024-03-30" ),
																																], [
								...
								useCallback( ( imageResource: Resource[] ) => {
												...
												if( selectedBookId ){
																dispatch( setPropToLoanState( {
																				...
																				value: (images ?? []).concat( imageResource.map( i => ({ ...i, file: undefined, referenceId: selectedBookId }) ) ),
																} ) );
												}
...
const EditableFields = memo( () => {
				return (
								<Grid size = { 6 } justifyContent = { "end" } container spacing = { 1 }>
												<AfterBorrowField name = { "currenBookBorrowing.extendDate" } type = { "date-time" } label = { appStrings.borrow.EXTEND_DATE } />
												<AfterBorrowField name = { "currenBookBorrowing.returnDate" } type = { "date-time" } label = { appStrings.borrow.NEXT_RETURN_DATE } />
												<ExtendButton />
												<AfterBorrowField name = { "currenBookBorrowing.fineType" } label = { appStrings.borrow.FINE_TYPE } />
												<AfterBorrowField name = { "currenBookBorrowing.noteAfterBorrow" } label = { appStrings.borrow.BOOK_DAMAGE_AFTER_BORROW } />
								</Grid>
				);
} );
const ExtendButton = memo( () => {
				...
				return (
								<Button loading = { isLoading } onClick = { onExtend } variant = "outlined" sx = { { borderColor: color.COMFORT, color: color.COMFORT } }>
...
const ConfirmReturnButton = memo( () => {
				...
				return (
								<Button fullWidth loading = { isLoading } variant = "contained" sx = { { bgcolor: color.COMFORT, color: color.LIGHT_TEXT, mt: 3 } } onClick = { onConfirmReturn }>
